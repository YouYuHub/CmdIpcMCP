...
namespace mcp_tools
{
  ...
  ToolInfo setup_pipe_info = []() -> ToolInfo
  {
    ...
    info.inputSchema = {
        {"type", "object"},
        {"properties", {{"pipe_name", {{"type", "string"}, {"description", R"(Pipe path \\.\pipe\name, default \\.\pipe\default_server)"}}}, {"terminal_mode", {{"type", "string"}, {"description", "Shell command, only used at session creation; default cmd.exe /k chcp 65001"}}}, {"first_command", {{"type", "string"}, {"description", "Optional first command, default is empty"}}}, {"wait_milliseconds", {{"type", "integer"}, {"description", "Max wait in ms; default 5000"}}}, {"prompt", {{"type", "string"}, {"description", "Match substring markers, separated by \"|\"; return early if one is found; does not support regular expressions. If not specified, return after a timeout has elapsed. Default is empty."}}}}},
        {"required", json::array()}};
    return info;
  }();
  ...
  ToolInfo run_command_info = []
  {
    ...
    info.description =
        ...
        "Concurrent calls on the same pipe are queued, Commands (services, etc.) that have been running for a long time will block the execution of subsequent commands."
    ...
    info.inputSchema = {
        {"type", "object"},
        {"properties", {{"command", {{"type", "string"}, {"description", "Command to run; line breaks = Enter. PowerShell: use ; not &"}}}, {"pipe_name", {{"type", "string"}, {"description", R"(Pipe path \\.\pipe\name, default \\.\pipe\default_server)"}}}, {"wait_milliseconds", {{"type", "integer"}, {"description", "Max wait in ms; default 5000"}}}, {"prompt", {{"type", "string"}, {"description", "Match substring markers, separated by \"|\"; return early if one is found; does not support regular expressions. If not specified, return after a timeout has elapsed. Default is empty."}}}}},
  ...
  ToolInfo read_output_info = []
  {
    ...
    info.inputSchema = {
        {"type", "object"},
        {"properties", {{"pipe_name", {{"type", "string"}, {"description", R"(Pipe path \\.\pipe\name)"}}}, {"offset", {{"type", "integer"}, {"description", "Skip N bytes back from the tail, default 0"}}}, {"max_length", {{"type", "integer"}, {"description", "Max bytes; default 4096"}}}, {"wait_milliseconds", {{"type", "integer"}, {"description", "Delay before reading in ms; default 0"}}}}},
        {"required", json::array()}};
    return info;
  }();
