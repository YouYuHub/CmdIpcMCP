...
namespace mcp_tools
{
    ...
    ToolInfo setup_pipe_info = []() -> ToolInfo {
        ...
        info.inputSchema = {
          ...
          {"properties", {
            ...
            {"first_command", {{"type", "string"}, {"description", "Optional first command"}}},
            {"wait_milliseconds", {{"type", "integer"}, {"description", "Max wait in ms; default 5000"}}},
            {"prompt", {{"type", "string"}, {"description", "Completion marker(s), '|'-separated; early-returns when one appears"}}}
          }},
          {"required", json::array()}
        };
        return info;
    ...
    ToolInfo run_command_info = [] {
        ...
        info.description =
            ...
            "Never two calls on the same pipe at once; parallelize with distinct "
        ...
        info.inputSchema = {
          ...
          {"properties", {
            ...
            {"pipe_name", {{"type", "string"}, {"description", R"(Pipe path \\.\pipe\name)"}}},
            {"wait_milliseconds", {{"type", "integer"}, {"description", "Max wait in ms; default 5000"}}},
            {"prompt", {{"type", "string"}, {"description", "Completion markers '|'-separated (e.g. \"C:\\\\>|PS C:\\\\>\"); prompt hit = done; set for long commands"}}}
          }},
    ...
    ToolInfo read_output_info = [] {
        ...
        info.inputSchema = {
          ...
          {"properties", {
            {"pipe_name", {{"type", "string"}, {"description", R"(Pipe path \\.\pipe\name)"}}},
            {"offset", {{"type", "integer"}, {"description", "Skip N bytes back from the tail (0 = newest)"}}},
