...
namespace mcp_tools
{
  ...
  ToolInfo setup_pipe_info = []() -> ToolInfo
  {
    ...
    info.inputSchema = {
        {"type", "object"},
        {"properties", {{"pipe_name", {{"type", "string"}, {"description", R"(Named pipe path, e.g. \\.\pipe\powershell_server)"}, {"default", R"(\\.\pipe\default_server)"}}}, {"terminal_mode", {{"type", "string"}, {"description", "Terminal command, e.g. cmd.exe /k chcp 65001 or powershell.exe. The terminal is launched with chcp 65001 for UTF-8 support."}, {"default", "cmd.exe /k chcp 65001"}}}, {"first_command", {{"type", "string"}, {"description", "Single command to execute, e.g. dir or ls."}, {"default", ""}}}, {"wait_milliseconds", {{"type", "integer"}, {"description", "Maximum wait time in ms; returns early if output stabilises (unchanged for ~1s) or prompt string is found in output."}, {"default", 30000}}}, {"prompt", {{"type", "string"}, {"description", "Optional prompt string; returns as soon as this string appears in output (e.g. root@host:~#). Can be used to detect command completion."}, {"default", ""}}}}},
        {"required", json::array()}};
    return info;
  }();
  ...
  ToolInfo run_command_info = []
  {
    ...
    info.inputSchema = {
        {"type", "object"},
        {"properties", {{"command", {{"type", "string"}, {"description", "Single command to execute, e.g. dir or ls. PowerShell: use ; not &."}}}, {"pipe_name", {{"type", "string"}, {"description", R"(Named pipe path, e.g. \\.\pipe\cmd_server)"}, {"default", R"(\\.\pipe\default_server)"}}}, {"wait_milliseconds", {{"type", "integer"}, {"description", "Maximum wait time in ms; returns early if output stabilises (unchanged for ~1s) or prompt string is found. Higher values allow longer-running commands to complete."}, {"default", 30000}}}, {"prompt", {{"type", "string"}, {"description", "Optional prompt string; returns as soon as this appears in output (e.g. C:\\Users\\YouYu>). Provides faster completion detection."}, {"default", ""}}}}},
  ...
  ToolInfo history_info = []
  {
    ...
    info.inputSchema = {
        {"type", "object"},
        {"properties", {{"pipe_name", {{"type", "string"}, {"description", R"(Named pipe path, e.g. \\.\pipe\pty_server)"}, {"default", R"(\\.\pipe\default_server)"}}}, {"offset", {{"type", "integer"}, {"description", "Skip N bytes from the end before reading backward. offset=0 reads the tail (most recent data); positive values read earlier blocks. If offset >= buffer size, returns empty (no data)."}, {"default", 0}}}, {"max_length", {{"type", "integer"}, {"description", "Maximum bytes to read backward from the offset position. Hard limit: 65535 (unsigned short). Larger values are clamped."}, {"default", 4096}}}, {"wait_milliseconds", {{"type", "integer"}, {"description", "Pre-read delay in ms (0 = read immediately). A simple Sleep before reading; use a positive value (e.g. 2000) to wait for a running command to finish producing output."}, {"default", 0}}}}},
        {"required", json::array()}};
    return info;
  }();
