...
namespace mcp_tools
{
  ...
  ToolInfo setup_pipe_info = []() -> ToolInfo
  {
    ...
    info.description =
        ...
        "y/N, vim) are answered by the user there - never inject secrets. "
        "terminal_mode applies only at session creation (fresh pipe_name to switch "
        "shells); a server restart loses shell state.";
    info.inputSchema = {
        {"type", "object"},
        {"properties", {{"pipe_name", {{"type", "string"}, {"description", R"(Pipe path \\.\pipe\name)"}}}, {"terminal_mode", {{"type", "string"}, {"description", "Shell command, only used at session creation; default cmd.exe /k chcp 65001"}}}, {"first_command", {{"type", "string"}, {"description", "Optional first command"}}}, {"wait_milliseconds", {{"type", "integer"}, {"description", "Max wait in ms; default 5000"}}}, {"prompt", {{"type", "string"}, {"description", "Completion marker(s), '|'-separated; early-returns when one appears"}}}}},
        {"required", json::array()}};
    return info;
  }();
  ...
  ToolInfo run_command_info = []
  {
    ...
    info.description =
        ...
        "Concurrent calls on the same pipe are queued; parallelize with distinct "
        "pipe_names. Completion: with prompt, returns when the prompt appears = "
        "command done; without, returns after wait_milliseconds regardless - "
        "unchanged output does NOT mean done, poll read_pipe_output. Line breaks = "
        "Enter (LF->CR): multi-line commands run line by line like typed; complex "
        "logic: write a script file and run it.";
    info.inputSchema = {
        {"type", "object"},
        {"properties", {{"command", {{"type", "string"}, {"description", "Command to run; line breaks = Enter. PowerShell: use ; not &"}}}, {"pipe_name", {{"type", "string"}, {"description", R"(Pipe path \\.\pipe\name)"}}}, {"wait_milliseconds", {{"type", "integer"}, {"description", "Max wait in ms; default 5000"}}}, {"prompt", {{"type", "string"}, {"description", "Completion markers '|'-separated (e.g. \"C:\\\\>|PS C:\\\\>\"); prompt hit = done; set for long commands"}}}}},
  ...
  ToolInfo read_output_info = []
  {
    ...
    info.description =
        "Read the session's output ring buffer (last 64 KB) backward from the tail; "
        "offset skips bytes back from the tail. Response carries "
        "[buffer_total_bytes: N] - compare across calls to detect new output. Mirrors "
        "the user's terminal window live. Read-only: errors if server not running (no "
        "auto-start).";
    info.inputSchema = {
        {"type", "object"},
        {"properties", {{"pipe_name", {{"type", "string"}, {"description", R"(Pipe path \\.\pipe\name)"}}}, {"offset", {{"type", "integer"}, {"description", "Skip N bytes back from the tail (0 = newest)"}}}, {"max_length", {{"type", "integer"}, {"description", "Max bytes; default 4096"}}}, {"wait_milliseconds", {{"type", "integer"}, {"description", "Delay before reading in ms; default 0"}}}}},
        {"required", json::array()}};
    return info;
  }();
